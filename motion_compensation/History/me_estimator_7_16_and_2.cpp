...
//            std::cout << err_0 << std::e ndl;
            ...
            int diap = 8;
            if (i % 16 == 0 && j % 16 == 0) {
                int x_prev = x_init, y_prev = y_init, x_new = 0, y_new = 0;
                while (!finished && diap != 2) {
                    for (int y = y_prev - diap; y <= y_prev + diap; y += diap) {
                        for (int x = x_prev - diap; x <= x_prev + diap; x += diap) {
                            auto comp = prev + y * width_ext + x;
                            int error = GetErrorSAD_16x16_my(cur, comp, width_ext);
                ...
                best_vector.x_16 = best_vector.x;
                best_vector.y_16 = best_vector.y;
                x_init = (best_vector.x), y_init = (best_vector.y);
            } else {
                int i_base = i - (i % 16), j_base = j - (j % 16);
                MV base_vector_16 = mvectors.get_mv(j_base, i_base);
                x_init = (base_vector_16.x_16), y_init = (base_vector_16.y_16);
            }
            ...
            diap = 4;
            for (int ind = 0; !finished && ind < pow_2(2 * diap + 1); ind++) {
                int x = x_init + spiral_4[ind].first, y = y_init + spiral_4[ind].second;
                const auto comp = prev + y * width_ext + x;
                const int error = GetErrorSAD_2x2_my(cur, comp, width_ext);
                if (error < best_vector.error) {
                    best_vector.x = x;
                    ...
                    best_vector.shift_dir = ShiftDir::NONE;
                    best_vector.error = error;
                    finished = (error < 30 * BLOCK_SIZE * BLOCK_SIZE);
                }
            }
