...
void MotionEstimator::CEstimate(const unsigned char *cur_Y,
    ...
    for (size_t i = 0; i < num_blocks_vert; i++) {
        for (size_t j = (i) & 1; j < num_blocks_hor; j += 2) {
            ...
            int x_prev = 0, y_prev = 0, x_acc = 0, y_acc = 0, x_vel = 0, y_vel = 0;
...
int GetErrorSAD_my(const unsigned char* block1, const unsigned char *block2, const int stride, const int block_size) {
    int sum = 0;
    for (int i = 0; i < block_size; ++i) {
        for (int j = 0; j < block_size; ++j) {
            sum += pow_2(int(block1[i * stride + j]) - int(block2[i * stride + j]));
        }
    }
    return sum;
}