...
void MotionEstimator::CEstimate(const unsigned char *cur_Y,
    ...
    for (size_t i = 0; i < num_blocks_vert; i += 2) {
        for (size_t j = 0; j < num_blocks_hor; j += 2) {
...
//            std::cout << err_0 << std::endl;
            ...
            // Brute force
            int x_init = 0, y_init = 0, diap;
            if (calc_disp(mvectors2, i, j) < quality) {
                auto elem = medium_elem_3x3(mvectors2, i, j);
                x_init = elem.first;
                y_init = elem.second;
                diap = 3;
            } else {
    ...
    for (size_t i = 0; i < num_blocks_vert; i += 1) {
        for (size_t j = (i & 1) ^1; j < num_blocks_hor; j += 1 << ((i & 1) ^ 1)) {
            ...
            int i_base = i & 0xfffffffe;
            int j_base = j & 0xfffffffe;
            std::pair<int, int>
                    p1 = elem(mvectors, i_base, j_base),
                    p2 = elem(mvectors, i_base + 2, j_base),
                    p3 = elem(mvectors, i_base, j_base + 2),
                    p4 = elem(mvectors, i_base + 2, j_base + 2);
            int x_init =
                    p1.first +
                    p2.first +
                    p3.first +
...
int GetErrorSAD_2x2_my(const unsigned char *block1, const unsigned char *block2, const int stride) {
    int sum = 0;
    sum += pow_2(int(block1[0 * stride + 0]) -int(block2[0 * stride + 0]));
    sum += pow_2(int(block1[0 * stride + 1]) -int(block2[0 * stride + 1]));
    sum += pow_2(int(block1[1 * stride + 0]) -int(block2[1 * stride + 0]));
...
int calc_disp(MEField &q, size_t i, size_t j) {
    int x_av = 0, y_av = 0;
    ...
    for (int y = j - 1; y <= j + 1; y++) {
        for (int x = i - 1; x <= i + 1; x++) {
            x_disp += (q.get_mv(y, x).x - x_av) * (q.get_mv(y, x).x - x_av);
            y_disp += (q.get_mv(y, x).y - y_av) * (q.get_mv(y, x).y - y_av);
        }
    }
