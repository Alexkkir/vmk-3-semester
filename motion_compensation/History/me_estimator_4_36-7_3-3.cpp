...
std::pair<int, int> MotionEstimator::medium_elem(MEField &q, size_t i, size_t j) const {
    int x = 0, y = 0;
    int diap = 2;
    for (size_t k = i - diap; k <= i + diap; k++) {
        for (size_t l = j - diap; l <= j + diap; l++) {
            x += elem(q, i, j).first;
            y += elem(q, i, j).second;
        }
    }
...
void MotionEstimator::CEstimate(const unsigned char *cur_Y,
    ...
    for (size_t i = 0; i < num_blocks_vert; ++i) {
        for (size_t j = i & 1; j < num_blocks_hor; j += 2) {
            ...
            // PUT YOUR CODE HERE
            int err_0 = GetErrorSAD_4x4_my(cur, prev_Y + vert_offset + hor_offset, width_ext);
//            std::cout << err_0 << std::endl;
            ...
            int finished = 0;
//            int x_init = medium_elem(mvectors2, i, j).first, y_init = medium_elem(mvectors2, i, j).second;
            ...
            for (int y = y_init - diap; !finished && y <= y_init + diap; y += 2) {
                for (int x = x_init - diap; !finished && x <= x_init + diap; x += 2) {
                    ...
                    const int error = GetErrorSAD_4x4_my(cur, comp, width_ext);
                    if (error < best_vector.error) {
                        best_vector.x = x;
                        best_vector.y = y;
                        best_vector.shift_dir = ShiftDir::NONE;
                        best_vector.error = error;
                        finished = (error < 22 * 16);
                    }
                }
            }
            ...
            diap = 4;
            for (int y = y_init - diap; y <= y_init + diap; y += 1) {
                for (int x = -x_init - diap; x <= x_init + diap; x += 1) {
                    const auto comp = prev + y * width_ext + x;
                    const int error = GetErrorSAD_4x4_my(cur, comp, width_ext);
                    if (error < best_vector.error) {
                        best_vector.x = x;
