...
// выполнение команды
int execute_command(char *dir, char** args, int fd1, int fd2);
...
int get_simplecmd(char* str, int fd1, int fd2)
{
    ...
    if(sons_counter == 1)
    {
        ...
        p = fork();
        if (!p)
            execute_command(dir, args, fd1, fd2);
        wait(&status);
        return status;
    }
    ...
    for (int i = 1; i < sons_counter;)
    {
        ...
        if (string[end] == '|')
        {
            ...
            p = fork();
            if (!p) 
                execute_command(dir, args, fdd0, fdd1);
...
int execute_command(char *dir, char** args, int fd1, int fd2)
{
    ...
    //printf("%s\n 0(%c) 1(%c) 2(%c)", args[0],args[0][0],args[0][1],args[0][2]);
    ...
    if (fd2 != 1) dup2(fd2, 1);
		...
		if(args[0][0]=='c' && args[0][1]=='d' && args[0][2] != '\0' && args[0][2] != '\n')
		{
			printf("IN IF CD_\n");
			//printf("%\n 0(%c) 1(%c) 2(%c)", str1[0],str1[1],str1[2]);
				...
				while(args[0][i+2] != '\n' && args[0][i+2] != '\0' && args[0][i+2] != '&' && args[0][i+2] != '|' && args[0][i+2] != ';')
				{
					str1[i]=args[0][i+2];
					printf("%c",str1[i]);
					i=i+1;
