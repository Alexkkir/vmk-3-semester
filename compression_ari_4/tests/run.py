...
DIR_TEST_FILES = os.path.join(DIR_ROOT, 'my_tests/public_tests_official')
...
def save_results(results, filename=FILE_RESULTS):
    header = [METHOD,
              ...
              COMPRESSED_SIZE,
              CONCLUSION]
    total_size_raw = 0; total_size_comp = 0
    for row in results:
        if row[CONCLUSION] == 'OK':
            total_size_raw += row[ORIGINAL_SIZE]
            total_size_comp += row[COMPRESSED_SIZE]
    try:
        total_coeff = round(total_size_comp / total_size_raw, 4)
    except BaseException:
        total_coeff = '-'
    with open(filename, 'w', newline='') as resfile:
        writer = csv.writer(resfile)
        writer.writerow(header + ['COEFF'])
        resfile.flush()
        for row in results:
            try:
                coeff = round(row.get(COMPRESSED_SIZE, 0) / row[ORIGINAL_SIZE], 4)
            except BaseException:
                coeff = '-'
            writer.writerow([row[column] for column in header]+[coeff])
            resfile.flush()
        writer.writerow([total_coeff])
