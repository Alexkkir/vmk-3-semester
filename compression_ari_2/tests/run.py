...
DIR_TEST_FILES = os.path.join(DIR_ROOT, TEST_DIRECTORY)
...
def save_results(results, filename=FILE_RESULTS):
    header = [METHOD,
              ...
              COMPRESSED_SIZE,
              CONCLUSION]
    sum_raw = 0; sum_comp = 0
    for row in results:
        sum_raw += row[ORIGINAL_SIZE]
        sum_comp += row[COMPRESSED_SIZE]
    coeff = sum_comp / sum_raw
    with open(filename, 'w', newline='') as resfile:
        writer = csv.writer(resfile)
        writer.writerow(header + ['COEFF'])
        resfile.flush()
        for row in results:
            writer.writerow([row[column] for column in header]+[round(row[COMPRESSED_SIZE] / row[ORIGINAL_SIZE], 4)])
            resfile.flush()
        # writer.writerow([sum_raw, sum_comp, round(coeff, 4)])
        writer.writerow([round(coeff, 4)])
